...
public class interceptor_movement : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Instantiate(smoke);
        GameObject.Find("smoke(Clone)").GetComponent<smoke_script>().index = index;
        GameObject.Find("smoke(Clone)").name = "smoke " + index;
    }
    ...
    void Update()
    {
        ...
        if (flight_stage == flight_stages.full_force)
        {
            ...
            if (distance_to_rocket < 5)
            {
                ...
                //GameObject.Find("smoke " + index).GetComponent<ParticleSystem>().Stop();
